...
class Account(object):
    ...
    def verification(self, url):
        ...
        verification = {"X-MAX-ACCESSKEY": self.key,
    ...
    def get_order(self, pair):
        path = f"{url}/api/v2/orders"
        return GET(path, headers=self.verification(path), data={"market": pair})

    # Post an order

    def create_order(self, market, side, volume, ord_type, price="", stop_price=""):
        path = f"{url}/api/v2/orders"
        order = dict({
            "market": market,
            "side": side,
    ...
    def delete_order(self, market, side):
